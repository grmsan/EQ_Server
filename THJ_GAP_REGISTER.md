...
| GAP-005 | AA-table dynamic timer preload parity | `extras/THJServer/zone/aa.cpp::Client::SendAlternateAdvancementTable`; `extras/THJServer/zone/aa.cpp::Client::GetDynamicAATimers` | `zone/aa.cpp::Client::SendAlternateAdvancementTable`; `zone/aa.cpp::{LoadDynamicAATimers, GetDynamicAATimer, SetDynamicAATimer}` | Must Port | Local code already has dynamic AA timer storage and lookup, but it does not perform the THJ-style cache preload before rebuilding the AA table, so timer assignment and presentation can lag behind THJ refresh behavior | `MC-FIX-03A` | Open |
...
- `2026-04-26` - `GAP-003`: `world/clientlist.cpp::GetMulticlassBitsOrBase()` now uses persisted multiclass ownership as-is after bucket lookup and only falls back to the legacy base-class bit when no persisted mask exists. Follow-up runtime proof should cover `CORE-01` and `UI-01` in [game_design/multiclass/TEST_TRACKER.md](game_design/multiclass/TEST_TRACKER.md).
