...
)
{
	...
	// --- Drop Tier Roll (Infinite Item Progression Step 7) ---
	// When enabled, each loot drop has a chance to be promoted to a higher tier.
	// Only base-tier items are eligible (don't double-roll already-tiered items).
	// The tiered version must exist in the DB (generated by tools/generate_tiered_items.py).
	if (RuleB(ItemProgression, DropTierEnabled) && !ItemProgression::IsTieredItem(item2->ID)) {
		float roll = zone->random.Real(0.0, 100.0);
		float mythic_thresh    = static_cast<float>(RuleR(ItemProgression, DropChanceMythic));
		float legendary_thresh = mythic_thresh + static_cast<float>(RuleR(ItemProgression, DropChanceLegendary));
		float enchanted_thresh = legendary_thresh + static_cast<float>(RuleR(ItemProgression, DropChanceEnchanted));

		int rolled_tier = ItemProgression::TierBase;
		if (roll <= mythic_thresh) {
			rolled_tier = ItemProgression::TierMythic;
		} else if (roll <= legendary_thresh) {
			rolled_tier = ItemProgression::TierLegendary;
		} else if (roll <= enchanted_thresh) {
			rolled_tier = ItemProgression::TierEnchanted;
		}

		if (rolled_tier > ItemProgression::TierBase) {
			uint32 tiered_id = ItemProgression::GetTieredItemID(item2->ID, rolled_tier);
			const EQ::ItemData *tiered_item = database.GetItem(tiered_id);
			if (tiered_item) {
				LogLoot(
					"Drop tier roll: NPC [{}] item [{}] ({}) rolled {} -> ID [{}] ({})",
					GetName(), item2->ID, item2->Name,
					ItemProgression::GetTierName(rolled_tier),
					tiered_id, tiered_item->Name
				);
				item2 = tiered_item;
			} else {
				LogLoot(
					"Drop tier roll: NPC [{}] item [{}] ({}) rolled {} but tiered ID [{}] not in DB",
					GetName(), item2->ID, item2->Name,
					ItemProgression::GetTierName(rolled_tier),
					tiered_id
				);
