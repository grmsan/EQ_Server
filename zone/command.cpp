...
int command_init(void)
{
	...
	if (
		...
		command_add("salvage", "(GM) Manually trigger Salvage Satchel processing", AccountStatus::Guide, command_salvage) ||
		command_add("essence", "[add N|set N] - (GM) Show or modify Essence currency balance", AccountStatus::Guide, command_essence) ||
		...
		command_add("removeclass", "[Class ID] - Removes a class from your or your targeted player's multiclass bitmask (must leave at least one class)", AccountStatus::GMAdmin, command_removeclass) ||
		...
		command_add("upgrade", "[slot_id] [levels] - Upgrade an equipped item using dynamic item scaling (default: cursor, 1 level)", AccountStatus::Guide, command_upgrade) ||
		...
		command_add("wppoc", "[list|travel <waypoint_id>|expedition] - Waypoint travel UI helper for custom client DLL POC", AccountStatus::Player, command_wppoc) ||
