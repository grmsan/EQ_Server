...
public:
    ...
    VOID SetSpell_Detour(int SpellID,bool HasSpellDescr)
    {
        ...
        if (!bNoSpellTramp) {
            ...
            strcpy(out,"<BR><c \"#00FFFF\">[Classless Mod]<br>");
        } else {
        ...
        if (pSpell->Mana > 0) {
            sprintf(temp, "Mana: %d<br>", pSpell->Mana);
            strcat(out, temp);
        }
        if (pSpell->CastTime > 0) {
             sprintf(temp, "CastTime: %1.2f<br>", (float)(pSpell->CastTime/1000.0f));
             strcat(out, temp);
        }
