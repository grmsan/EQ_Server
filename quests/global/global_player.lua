-- Global player script: notify player when they consider (target-check) a rare npc
-- This shows a loud message to the player and optionally alerts nearby players

-- Configs
local CONSIDER_ALERT_ENABLED = true
local CONSIDER_ALERT_RADIUS = 80 -- units, radius for local emote to nearby players

function event_consider(e)
    if not CONSIDER_ALERT_ENABLED then
        return
    end
    ...
    -- Only handle NPCs
    local ok_isnpc, is_npc = pcall(function() return other:IsNPC() end)
    if not ok_isnpc or not is_npc then
        return
    end

    -- Check if the mob is a named/rare spawn
    local ok_rare, is_rare = pcall(function() return other:CastToNPC():IsRareSpawn() end)
    if not ok_rare or not is_rare then
        return
    end

    -- Build alert message
    local ok_name, name = pcall(function() return other:GetCleanName() end)
    local ok_level, level = pcall(function() return other:GetLevel() end)
    name = ok_name and name or "Unknown"
    level = ok_level and level or 0
    local msg = string.format("!!! RARE MOB: %s (Level %d) !!!", name, level)

    -- Show to the player (clear, attention-grabbing)
    client:Message(MT.Red, msg)
    -- marquee for the player (big text overlay)
    if client.SendMarqueeMessage then
        client:SendMarqueeMessage(15, 510, 1, 100, 5000, msg)
    end

    -- Optionally alert nearby players in a radius so a party can respond
    if CONSIDER_ALERT_RADIUS > 0 and eq.local_emote then
        eq.local_emote({other:GetX(), other:GetY(), other:GetZ()}, MT.Magenta, CONSIDER_ALERT_RADIUS, msg)
    end
end

function event_consider_corpse(e)
    -- Consider corpse: treat same way if it's a rare NPC corpse
    event_consider(e)
end
