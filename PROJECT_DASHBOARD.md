...
|------|------|-----------------|------------|
| 2026-04-26 | Multiclass / AA parity | Closed the documented `MC-FIX-03A` gap in `zone/aa.cpp`: AA table rebuilds now preload persisted dynamic AA timer assignments before serializing ranks, and the tracker now includes `AA-02` to validate cooldown-family stability after zoning, relogging, or class mutation. | Rebuild `zone`, then run `AA-02` plus `AA-01`/`MC-VAL-02` to confirm the live AA window keeps stable shared cooldown families across AA table refreshes. |
