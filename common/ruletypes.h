...
RULE_INT(Character, AAMinimumLevel, 10, "Minimum level required to earn or allocate AA experience (set to 1 to allow AA XP from level 1).")
...
RULE_INT(Character, ItemManaRegenCap, 1500, "Limit on mana regeneration granted by items")
RULE_INT(Character, ItemHealthRegenCap, 3000, "Limit on health regeneration granted by items")
...
RULE_INT(Character, ItemATKCap, 1250, "Limit on ATK granted by items")
RULE_INT(Character, ItemHealAmtCap, 1250, "Limit on heal amount granted by items")
RULE_INT(Character, ItemSpellDmgCap, 1250, "Limit on spell damage granted by items")
...
RULE_INT(Character, StatCap, 9999, "If StatCap > 0 then this value is used. If it is 0, the value of the following code is used: If Level < 61: 255. If Level >= 61 and the client SoF or newer: 255 + 5 x (level -60).  If the client is older than SoF and the level < 71 then: 255 + x (level-60). In all other cases: 330.")
...
RULE_BOOL(Character, UseNoJunkFishing, false, "Disregards junk items when fishing")
	...
	RULE_BOOL(Character, EnableServerStatsUpdate, false, "Send custom OP_ServerStatsUpdate packets (requires a custom client/DLL that understands the packet).")
	RULE_BOOL(Character, EnableEdgeStatLabel, false, "Send custom EdgeStatLabel (opcode 0x1338) key/value stat packets (requires a custom client/DLL that understands the packet).")
...
RULE_INT(Character, BandolierSwapDelay, 0, "Bandolier swap delay in milliseconds, default is 0")
	...
	RULE_BOOL(Character, EnableHackedFastCamp, false, "Enables hacked fast camp for all clients (testing).")
	RULE_INT(Character, HackedFastCampTimerMS, 100, "Camp timer in milliseconds when EnableHackedFastCamp is enabled (testing).")
...
RULE_CATEGORY(Pets)
RULE_REAL(Pets, AttackCommandRange, 350, "Range at which a pet will respond to attack commands")
RULE_BOOL(Pets, UnTargetableSwarmPet, false, "Setting whether swarm pets should be targetable")
RULE_REAL(Pets, PetPowerLevelScale, 0.60, "Levels gained per point of pet power for pets that use dynamic petpower scaling rows")
RULE_REAL(Pets, PetPowerLevelCap, 15, "Maximum number of levels a player pet can go up with pet power")
...
RULE_BOOL(Spells, EnableIntWisWeightedExtraSpellBonus, false, "Enable INT/WIS-based scaling on top of SpellDmg/HealAmt bonus amounts. This scales only the extra bonus portion, not base spell value. Default false preserves legacy SpellDmg/HealAmt behavior.")
RULE_REAL(Spells, IntWisDamageSecondaryWisScalar, 0.20, "For damage bonus scaling: WIS contributes this fraction in addition to INT (stat_score = INT + WIS * scalar)")
RULE_REAL(Spells, IntWisHealingSecondaryIntScalar, 0.20, "For healing bonus scaling: INT contributes this fraction in addition to WIS (stat_score = WIS + INT * scalar)")
RULE_REAL(Spells, IntWisExtraSpellBonusStatDivisor, 1200.0, "Stat divisor for INT/WIS weighted bonus scaling. Lower values make scaling stronger.")
RULE_REAL(Spells, IntWisExtraSpellBonusStatExponent, 1.0, "Exponent applied to normalized INT/WIS stat score for weighted bonus scaling.")
RULE_BOOL(Spells, IntWisScaleExtraSpellAmtByBase, true, "Scale INT/WIS bonus effect by spell base value so low-base spells get less benefit and high-base spells get more.")
RULE_INT(Spells, IntWisExtraSpellBonusMinBase, 150, "Base spell value where INT/WIS weighted bonus scaling starts ramping in.")
RULE_INT(Spells, IntWisExtraSpellBonusMaxBase, 2500, "Base spell value where INT/WIS weighted bonus scaling reaches full strength.")
RULE_REAL(Spells, IntWisExtraSpellBonusBaseExponent, 1.35, "Exponent for base-value weighting ramp. >1 favors higher-base spells more strongly.")
RULE_REAL(Spells, IntWisExtraSpellBonusMultiplierCap, 1.75, "Hard cap for total multiplier applied to SpellDmg/HealAmt extra bonus after INT/WIS scaling.")
...
RULE_BOOL(Combat, UseHotfixableCombatBalance, false, "Enable hotfixable combat balance values from config file. FALSE = use compile-time constants in combat_balance_config.h, TRUE = use runtime values from combat_balance.ini.")
...
RULE_BOOL(Combat, UseNewStaminaFormula, false, "Enable new STA-based sustainability system (HP scaling, regen, mitigation). FALSE = use legacy formulas, TRUE = use new STA scaling from combat_balance_config.h. Toggle for A/B testing during development.")
RULE_BOOL(Combat, UseNewDexFormulas, true, "Enable new DEX-based precision system (crits, procs, bow force, twincast, penetration) using combat_balance_config.h tuning knobs.")
RULE_BOOL(Combat, UseNewAgiFormulas, true, "Enable new AGI-based speed/avoid formulas (haste, avoidance, run speed, cast speed) using combat_balance_config.h tuning knobs.")
...
RULE_BOOL(Items, LoreEquippedOnly, false, "Enable this to allow duplicate lore items in inventory and enforce lore conflicts only on equipment swaps.")
...
RULE_BOOL(Items, EnableCustomItemStats, true, "Enable per-instance custom item stat scaling/overrides (dynamic_level and ItemInstance custom data).")
RULE_BOOL(Items, SendCustomItemStatsToClient, false, "Append per-instance custom stat payloads to RoF2 item packets (requires a custom client DLL to consume).")
RULE_CATEGORY_END()
...
// Power Slot XP — kill-based tier progression (Step 3)
RULE_INT(ItemProgression, BaseItemXP, 40, "Base item XP awarded per kill for Power Slot item.")
RULE_INT(ItemProgression, TierThresholdEnchanted, 1000, "Item XP required to tier up from Base to Enchanted.")
RULE_INT(ItemProgression, TierThresholdLegendary, 20000, "Item XP required to tier up from Enchanted to Legendary.")
RULE_INT(ItemProgression, TierThresholdMythic, 150000, "Item XP required to tier up from Legendary to Mythic.")
RULE_REAL(ItemProgression, ConMultGrey, 0.0f, "Item XP multiplier for grey-con mobs.")
RULE_REAL(ItemProgression, ConMultGreen, 0.125f, "Item XP multiplier for green-con mobs.")
RULE_REAL(ItemProgression, ConMultLightBlue, 0.375f, "Item XP multiplier for light-blue-con mobs.")
RULE_REAL(ItemProgression, ConMultBlue, 0.75f, "Item XP multiplier for blue (dark blue)-con mobs.")
RULE_REAL(ItemProgression, ConMultWhite, 1.0f, "Item XP multiplier for white-con mobs.")
RULE_REAL(ItemProgression, ConMultYellow, 1.125f, "Item XP multiplier for yellow-con mobs.")
RULE_REAL(ItemProgression, ConMultRed, 1.25f, "Item XP multiplier for red-con mobs.")
RULE_REAL(ItemProgression, NamedXPMult, 3.0f, "Item XP multiplier for named (rare_spawn) NPCs.")
RULE_REAL(ItemProgression, RaidTierXPMult, 5.0f, "Item XP multiplier for raid-tier NPCs (level 55+).")
RULE_INT(ItemProgression, RaidTierMinLevel, 55, "Minimum NPC level to qualify for raid-tier item XP multiplier.")
...
RULE_INT(Custom, FarmingInstanceTemplateVersion, 0, "Template version backing respawning farming instances.")
...
RULE_BOOL(Custom, EnablePetBags, true, "Enable THJ-style pet bag equipment sync for class-based pets.")
RULE_BOOL(Custom, MultipleTwoHandedProcs, true, "Allow two-handed weapons to proc multiple procs (such as augments).")
RULE_BOOL(Custom, ExcludeTempPetsFromProcChanceSPA, false, "Prevent SE_ProcChance from affecting procs that spawn temporary pets.")
RULE_BOOL(Custom, DoubleAttackSkillRanged, true, "Use Double and Triple Attack skills for ranged attacks.")
RULE_BOOL(Custom, ApplyPetAAToSwarm, true, "Apply pet AA effects to swarm pets.")
RULE_REAL(Custom, ScaleAutoAttackByHStr, 1.0f, "Scale auto attack damage by this value. 0.0 to disable.")
RULE_INT(Custom, ScaleAutoAttackHStrSoftCap, 100, "Soft-cap: lower bound of heroic STR linear decay scaling.")
RULE_REAL(Custom, ScaleAutoAttackHStrScaleFloor, 0.25f, "Minimum scaling floor applied past heroic STR soft-cap.")
RULE_REAL(Custom, ScaleAutoAttackHStrScaleFactor, 0.0075f, "Rate at which scaling decays per heroic STR above soft-cap.")
RULE_REAL(Custom, ScaleBowByHDex, 1.0f, "Scale bow damage by heroic DEX. 0.0 to disable.")
RULE_REAL(Custom, ScaleBowByHDexDivide, 6.66f, "Divide final heroic DEX bow contribution by this amount.")
RULE_INT(Custom, ScaleBowHDexSoftCap, 200, "Soft-cap: lower bound of heroic DEX linear decay scaling.")
RULE_REAL(Custom, ScaleBowHDexScaleFloor, 0.15f, "Minimum scaling floor applied past heroic DEX soft-cap.")
RULE_REAL(Custom, ScaleBowHDexScaleFactor, 0.0125f, "Rate at which bow scaling decays per heroic DEX above soft-cap.")
RULE_REAL(Custom, ScaleBowMinimumDamageMultiplier, 5.0f, "Divide bow+arrow damage by this amount, multiply by divided heroic DEX for bow minimum damage.")
RULE_REAL(Custom, ScaleBowMinimumDamageDivisor, 4.0f, "Divide heroic DEX by this amount before applying bow minimum damage multiplier.")
RULE_REAL(Custom, DevastatingFrenzyDamageMultiplier, 1.0f, "Apply this multiplier for each 20% missing HP from target when Devastating Frenzy triggers.")
