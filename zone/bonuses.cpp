...
void Client::CalcBonuses()
{
	...
	const auto old_max_hp   = max_hp;
	const auto old_max_mana = max_mana;
	const auto old_max_end  = max_end;

	CalcMaxHP();
	CalcMaxMana();
	CalcMaxEndurance();

	// Keep the client UI synchronized when max pools change (buffs, gear, AAs).
	if (old_max_hp != max_hp || old_max_mana != max_mana || old_max_end != max_end) {
		SendHPUpdate(true);
	}
	...
	if (!is_monk && IsClient() && RuleB(Custom, MulticlassingEnabled)) {
		is_monk = CastToClient()->HasClass(Class::Monk);
	}
	int timer = is_monk ? CONSUMPTION_MNK_TIMER : CONSUMPTION_TIMER;
